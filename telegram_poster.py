...
)  # Model supporting strict function calling, ALWAYS should be gpt-4.1
MAX_CONTEXT_CHARS = int(
    os.getenv("MAX_CONTEXT_CHARS", 15000)
)  # Approximate limit to avoid huge API requests (adjust as needed)
...
if OPENAI_API_KEY and OPENAI_API_KEY != "YOUR_OPENAI_API_KEY":
    ...
    logging.warning(
        "OPENAI_API_KEY not found or is placeholder. LLM features will be disabled."
    )
        ...
        try:
            with open(STATE_FILE, "r", encoding="utf-8") as f:
                ...
                if "last_poll_message_id" not in state:
                    state["last_poll_message_id"] = default_state[
...
def save_state(state):
    if DRY_RUN:
        logging.info("DRY_RUN is true, not saving state")
        return
...
def validate_config():
    ...
    if not openai_client:
        logging.warning(
            "OpenAI client is not initialized. Check OPENAI_API_KEY. LLM features disabled."
        )
    ...
    if not GEMINI_API_KEY or not GEMINI_IMAGE_MODEL or not IMAGE_PROMPT_START:
        logging.error("GEMINI_API_KEY and GEMINI_IMAGE_MODEL and IMAGE_PROMPT_START cannot be empty.")
        valid = False
    return valid
...
def generate_story_continuation_openai(
    current_story: str, user_choice: str
) -> str | None:
...
async def get_poll_winner(bot: Bot, chat_id: str | int, message_id: int) -> str | None:
    ...
    try:
        ...
        if "poll has already been closed" in err_text:
            # Use logging
            logging.info(
                f"Poll (ID: {message_id}) was already closed. Attempting to fetch results directly (Currently not reliably implemented).",
                exc_info=True,
            )
...
def make_gemini_image(text: str):
    client = genai.Client(api_key=GEMINI_API_KEY)

    prompt = (
        f"{IMAGE_PROMPT_START} {text}"
    )
    ...
    try:
        ...
        if last_poll_message_id:
            ...
            if poll_winner:
                ...
                next_prompt = (
                    INITIAL_STORY_IDEA  # Fallback if poll fails or has no winner
                )
        else:
            # Use logging
        ...
        if not current_story:  # Is this the very first run?
            ...
            new_story_part = generate_story_continuation_openai(
                current_story, next_prompt
            )
            logging.info("Generating image...")
            image = make_gemini_image(new_story_part)

            if (
                new_story_part and new_story_part.strip()
            ):  # Check if LLM returned something valid
                # Use logging
                ...
                raise RuntimeError(
                    "LLM failed to generate story continuation."
        ...
        if not poll_options or len(poll_options) != 4:
            ...
            try:
                reply_params = ReplyParameters(text_message.id) if text_message else None
...
if __name__ == "__main__":
    ...
    if not validate_config():
        logging.critical(
            "Configuration validation failed. Please check BOT_TOKEN, CHANNEL_ID, and OPENAI_API_KEY (if used). Exiting."
        )
    elif not openai_client:
        logging.warning(
            "OpenAI client not initialized. LLM features will use placeholders or fail. Proceeding..."
        )
